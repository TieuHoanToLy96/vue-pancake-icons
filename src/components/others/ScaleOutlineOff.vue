<script>
export default {
  install(Vue, options) {
    Vue.component('scale-outline-off-pcon', component)
  },
  name: 'scale-outline-off',
  props: {
    size: {
      type: Number,
      default: 24
    },
    width: {
      type: Number,
      default: 0
    },
    height: {
      type: Number,
      default: 0
    },
    strokeWidth: {
      type: Number,
      default: 1.7
    },
    color: {
      type: String,
      default: "currentColor"
    }
  }
}
</script>
  
<template>
  <svg :style='{color: color}' :width="width || size" :height="height || size" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<path d="M7 3H17C18.0609 3 19.0783 3.42143 19.8284 4.17157C20.5786 4.92172 21 5.93913 21 7V17M19.827 19.83C19.456 20.2016 19.0153 20.4962 18.5302 20.697C18.0451 20.8977 17.525 21.0007 17 21H7C5.93913 21 4.92172 20.5786 4.17157 19.8284C3.42143 19.0783 3 18.0609 3 17V7C3 5.896 3.447 4.897 4.17 4.173" stroke="currentColor" :stroke-width="strokeWidth" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M11.0619 7.062C11.3719 7.021 11.6839 7 11.9999 7C13.9559 7 15.7239 7.802 16.9939 9.095C16.3231 9.72357 15.6584 10.3586 14.9999 11M11.2769 11.288C10.7857 11.41 10.3333 11.6542 9.96186 11.998L7.00586 9.095C7.35204 8.74139 7.73486 8.42561 8.14786 8.153" stroke="currentColor" :stroke-width="strokeWidth" stroke-linecap="round" stroke-linejoin="round"/>
<path d="M3 3L21 21" stroke="currentColor" :stroke-width="strokeWidth" stroke-linecap="round" stroke-linejoin="round"/>
</svg>

</template>
    